...
# Filter
df_2 <- df %>% select(Trade,
                      Cdis,Mdis,RelDis,MRelDis,
                      L,Adj,MerSul,
                      
                      #Exp
                      Exp_SizeCou,Exp_SizeWat,Exp_AgrLan,Exp_UrbLan,Exp_ForLan,
                      Exp_Pop,Exp_RurPop,Exp_UrbPop,Exp_OldPop,Exp_PopDen,
                      Exp_Gdp,Exp_GdpPer,Exp_Unem,Exp_LabFor,
                      Exp_DomCom,Exp_STV,Exp_MarCap,Exp_LTE,
                      Exp_LifeExp,Exp_MorRa,Exp_DeaRa,Exp_FerRa,Exp_CoEmi,
                      
                      #Imp
                      Imp_SizeCou,Imp_SizeWat,Imp_AgrLan,Imp_UrbLan,Imp_ForLan,
                      Imp_Pop,Imp_RurPop,Imp_UrbPop,Imp_OldPop,Imp_PopDen,
                      Imp_Gdp,Imp_GdpPer,Imp_Unem,Imp_LabFor,
                      Imp_DomCom,Imp_STV,Imp_MarCap,Imp_LTE,
                      Imp_LifeExp,Imp_MorRa,Imp_DeaRa,Imp_FerRa,Imp_CoEmi)
...
df_2 <- df_2 %>% mutate(Trade = log(Trade),
                        ...
                        Exp_AgrLan=log(Exp_AgrLan),
                        Exp_UrbLan=log(Exp_UrbLan),
                        Exp_ForLan=log(Exp_ForLan),
                        Exp_Pop=log(Exp_Pop),
                        Exp_RurPop=log(Exp_RurPop),
                        Exp_UrbPop=log(Exp_UrbPop),
                        Exp_OldPop=log(Exp_OldPop),
                        Exp_PopDen=log(Exp_PopDen),
                        Exp_Gdp=log(Exp_Gdp),
                        Exp_GdpPer=log(Exp_GdpPer),
                        Exp_Unem=log(Exp_Unem),
                        Exp_LabFor=log(Exp_LabFor),
                        Exp_DomCom=log(Exp_DomCom),
                        Exp_STV=log(Exp_STV),
                        Exp_MarCap=log(Exp_MarCap),
                        Exp_LTE=log(Exp_LTE),
                        Exp_LifeExp=log(Exp_LifeExp),
                        Exp_MorRa=log(Exp_MorRa),
                        Exp_DeaRa=log(Exp_DeaRa),
                        Exp_FerRa=log(Exp_FerRa),
                        Exp_CoEmi=log(Exp_CoEmi),
                        
                        
                        Imp_SizeCou=log(Imp_SizeCou),
                        Imp_SizeWat=log(Imp_SizeWat),
                        Imp_AgrLan=log(Imp_AgrLan),
                        Imp_UrbLan=log(Imp_UrbLan),
                        Imp_ForLan=log(Imp_ForLan),
                        Imp_Pop=log(Imp_Pop),
                        Imp_RurPop=log(Imp_RurPop),
                        Imp_UrbPop=log(Imp_UrbPop),
                        Imp_OldPop=log(Imp_OldPop),
                        Imp_PopDen=log(Imp_PopDen),
                        Imp_Gdp=log(Imp_Gdp),
                        Imp_GdpPer=log(Imp_GdpPer),
                        Imp_Unem=log(Imp_Unem),
                        Imp_LabFor=log(Imp_LabFor),
                        Imp_DomCom=log(Imp_DomCom),
                        Imp_STV=log(Imp_STV),
                        Imp_MarCap=log(Imp_MarCap),
                        Imp_LTE=log(Imp_LTE),
                        Imp_LifeExp=log(Imp_LifeExp),
                        Imp_MorRa=log(Imp_MorRa),
                        Imp_DeaRa=log(Imp_DeaRa),
                        Imp_FerRa=log(Imp_FerRa),
                        Imp_CoEmi = log(Imp_CoEmi)
...
correlation_accuracy <- cor(actuals_preds)
min_max_accuracy <- mean(apply(actuals_preds, 1, min) / apply(actuals_preds, 1, max))  
mape <- mean(abs((actuals_preds$X1 - actuals_preds$actuals))/actuals_preds$actuals)  
